...
class MyLaughterDataset(torch.utils.data.Dataset):
    def __init__(self, df, hash_to_file, feature_fn, sr, subsample=True):
        # df: loaded dataframe with columns ['offset','duration','subsampled_offset','subsampled_duration','audio_path','label']
        # dict mapping an audio_path (hash) to a raw audio
        # feature_fn: feature extraction method called for every sample (if provided)
        # sr: passed to feature_fn
        # subsample:
