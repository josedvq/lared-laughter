...
laugh_keys = laughter_tag_map.keys()
...
def get_laughter_infolist(csv_file, mode):
	...
	for l in lines:
		...
		laugh_tags = [laughter_tag_map[t] for t in tag_strings if t in laugh_keys]
		if laugh_tags:
			laughter_lines.append({'yt_id': yt_id, 'start_time': start_time,
				'end_time': end_time, 'tag_strings': tag_strings, 'laughter': True})
		else:
			all_non_laughter_lines.append({'yt_id': yt_id, 'start_time': start_time,
				'end_time': end_time, 'tag_strings': tag_strings, 'laughter': False})
...
def download_laughter_audio_file(laughter_info, destination_dir):
	...
	try:
		...
		shutil.move(downloaded_file, tmp_mp3_file)
		...
		shutil.move(tmpdir+mp3_file, destination_file)
		# remove tmpdir
		os.rmdir(tmpdir)
	except:
